  ...
  >
    ...
    <div>
      <div class="flex items-center mb-4">
        ...
        <button
          class="p-2 ml-4 mr-2 border-2 rounded flex-no-shrink hover:text-white text-green border-green hover:bg-green"
        >
          Done
        </button>
        <button
          class="p-2 ml-2 border-2 rounded flex-no-shrink text-red border-red hover:text-white hover:bg-red"
        >
          Remove
        </button>
      </div>
      <div class="flex items-center mb-4">
        ...
        <button
          class="p-2 ml-4 mr-2 border-2 rounded flex-no-shrink hover:text-white text-grey border-grey hover:bg-grey"
        >
          Not Done
        </button>
        <button
    ...
    <section class="bg-white dark:bg-gray-900">
      <div class="max-w-2xl px-4 py-8 mx-auto lg:py-16">
        ...
        <form>
          ...
          <button
            @click.prevent="todoItemsStore.setDrillName('test')"
            type="submit"
    ...
    <p v-for="(todo, index) in todos" :key="index">
      {{ index }}: {{ todo }}
      <button @click="testStore.removeTodo(index)">delete</button>
    </p>
  </div>
</template>
...
import {
  ...
  useCounterStore,
  useMainStore,
} from '@/stores/todoitems.store';
import { computed, ref } from 'vue';

const todoItemsStore = useToDoItemsStore();

const testStore = useMainStore();

const mytodo = ref('');

const todos = computed(() => testStore.getAllTodos);

const AddTodo = () => {
  if (mytodo.value != '') {
    testStore.addTodo(mytodo.value);
    mytodo.value = '';
  }
};
</script>
